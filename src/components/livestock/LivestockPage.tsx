...
export function LivestockPage() {
  ...
  const { animals, animalProducts, addAnimal, deleteAnimal, addProduct, deleteProduct } = useStore(
    useShallow((s) => ({
      animals: s.animals, animalProducts: s.animalProducts,
      addAnimal: s.addAnimal, deleteAnimal: s.deleteAnimal,
      addProduct: s.addProduct, deleteProduct: s.deleteProduct,
    }))
  );

  const [tab, setTab] = useState<"animals" | "production">("animals");
  ...
  const stats = useMemo(() => {
    ...
    return { totalAnimals, eggsThisWeek, honeyThisYear };
  }, [animals, animalProducts, weekStart, weekEnd, now]);
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={() => tab === "animals" ? setShowAddAnimal(true) : setShowAddProduct(true)}>
            <Plus size={16} />
            {tab === "animals" ? t("livestock.addAnimal") : t("livestock.addProduct")}
          </Button>
        </div>
      </div>
